...
def load_features(config: dict, debug_mode=False) -> Tuple[pd.DataFrame, Optional[pd.DataFrame]]:
    feathre_path = pathlib.Path(config['feature_dir_path'])

    train_dfs = [pd.read_feather(feathre_path / f'{fe}_train.ftr') for fe in config['features']]
    x_train = pd.concat(train_dfs, axis=1)

    if debug_mode:
        return x_train, None
    else:
        test_dfs = [pd.read_feather(feathre_path / f'{fe}_test.ftr') for fe in config['features']]
